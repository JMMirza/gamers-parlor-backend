    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Create New Wager Post</h4>
            </div>
            <div class="card-body">
                <div class="live-preview">
                    <form class="row g-3 needs-validation" novalidate
                        action="{{ route('wager-post.update', $wagerPost->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-4 col-sm-12">
                            <div class="form-label-group in-border">
                                <select class="form-select form-control mb-3" name="game_id" required>
                                    <option value="" @if ($wagerPost->game_id == '') {{ 'selected' }} @endif
                                        selected disabled>
                                        Select One
                                    </option>
                                    @foreach ($games as $game)
                                        <option value="{{ $game->id }}"
                                            @if ($wagerPost->game_id == $game->id) {{ 'selected' }} @endif>
                                            {{ $game->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4 col-sm-12">
                            <div class="form-label-group in-border">
                                <select class="form-select form-control mb-3" name="platform_id" required>
                                    <option value="" @if ($wagerPost->platform_id == '') {{ 'selected' }} @endif
                                        selected disabled>
                                        Select One
                                    </option>
                                    @foreach ($platforms as $platform)
                                        <option value="{{ $platform->id }}"
                                            @if ($wagerPost->platform_id == $platform->id) {{ 'selected' }} @endif>
                                            {{ $platform->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-label-group in-border">
                                <div class="input-group">
                                    <input type="date" class="form-control" data-provider="flatpickr"
                                        ...
                                        data-deafult-date="{{ date('Y-m-d') }}" value="{{ $wagerPost->start_date }}"
                        ...
                        <div class="col-md-6 col-sm-12">
                            <div class="form-label-group in-border">
                                <select class="form-select form-control mb-3" name="status_id" required>
                                    <option value="" @if ($wagerPost->status_id == '') {{ 'selected' }} @endif
                                        selected disabled>
                                        Select One
                                    </option>
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status->id }}"
                                            @if ($wagerPost->status_id == $status->id) {{ 'selected' }} @endif>
                                            {{ $status->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-12 col-sm-12 mb-3">
                            <div>
                                <textarea class="form-control ckeditor-description" id="terms_and_condition" placeholder="Please enter"
                                    name="terms_and_condition">{{ $wagerPost->terms_and_condition }}</textarea>
                            </div>
                        </div>
