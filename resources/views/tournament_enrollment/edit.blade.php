    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Update Enrollment</h4>
            </div>

            <div class="card-body">
                <form class="row  needs-validation" action="{{ route('tournament-enrollments.update', $enrollment->id) }}"
                    ...
                    <div class="col-md-6">
                        <div class="form-label-group in-border">
                            <select class="form-select form-control mb-3" name="tournament_id" required>
                                <option value="" @if ($enrollment->tournament_id == '') {{ 'selected' }} @endif
                                    selected disabled>
                                    Select One
                                </option>
                                @foreach ($tournaments as $tournament)
                                    <option value="{{ $tournament->id }}"
                                        @if ($enrollment->tournament_id == $tournament->id) {{ 'selected' }} @endif>
                                        {{ $tournament->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-label-group in-border">
                            <select class="form-select form-control mb-3" name="team_id" required>
                                <option value="" @if ($enrollment->team_id == '') {{ 'selected' }} @endif
                                    selected disabled>
                                    Select One
                                </option>
                                @foreach ($teams as $team)
                                    <option value="{{ $team->id }}"
                                        @if ($enrollment->team_id == $team->id) {{ 'selected' }} @endif>
                                        {{ $team->name }}
                                    </option>
                                @endforeach
                            </select>
